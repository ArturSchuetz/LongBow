...
void WaveletSphere::Init(Bow::Renderer::ShaderProgramPtr shaderProgram, const char* heighMapPath)
{
	...
	for (unsigned int i = 0; i < m_RootTriangles.size(); ++i)
	{
		RecursCalculateScalar(&(m_RootTriangles[i]), 0, 0.00001f);
	}
}
...
void WaveletSphere::Render(Bow::Renderer::RenderContextPtr renderContext, Bow::Renderer::Camera* camera, bool renderFilled)
{
	if (m_isDirty)
	{
		...
		for (unsigned int i = 0; i < m_RootTriangles.size(); ++i)
		{
			RecursRender(&(m_RootTriangles[i]), positionsAttribute, colorAttribute);
		}
...
void WaveletSphere::RecursRender(WaveletTriangle* triangle, Bow::Core::VertexAttributeFloatVec3 *positionsAttribute, Bow::Core::VertexAttributeFloat *colorAttribute)
{
	if (triangle->HasChildren() && !triangle->ChildrenAreWavelets())
	{
		RecursRender(triangle->m_T0, positionsAttribute, colorAttribute);
		RecursRender(triangle->m_T1, positionsAttribute, colorAttribute);
		RecursRender(triangle->m_T2, positionsAttribute, colorAttribute);
		RecursRender(triangle->m_T3, positionsAttribute, colorAttribute);
	}
	else
	{
...
void WaveletSphere::RecursCalculateScalar(WaveletTriangle* triangle, int forceLevel, float threshold)
{
	if (triangle->HasChildren() && triangle->ChildrenAreWavelets())
	{
		if (m_Values[triangle->m_T1->m_ValueIndex] * m_Values[triangle->m_T1->m_ValueIndex] + 
			m_Values[triangle->m_T2->m_ValueIndex] * m_Values[triangle->m_T2->m_ValueIndex] +
			m_Values[triangle->m_T3->m_ValueIndex] * m_Values[triangle->m_T3->m_ValueIndex] > threshold * threshold || forceLevel > 0)
		{
