...
void WaveletSphere::Render(Bow::Renderer::RenderContextPtr renderContext, Bow::Renderer::Camera* camera)
{
	...
	m_shaderProgram->VSetUniform("u_terrainHeight", 0.00138879297f);
...
void WaveletSphere::RenderDetail(Bow::Renderer::RenderContextPtr renderContext, Bow::Renderer::Camera* camera)
{
	if (m_isDetailDirty)
	{
		...
		for (unsigned int i = 0; i < m_RootTriangles.size(); ++i)
		{
			RecursRenderDetail(&(m_RootTriangles[i]), indices);
		}
...
void WaveletSphere::RecursRenderDetail(WaveletTriangle* triangle, Bow::Core::IndicesUnsignedInt *indices)
{
	if (triangle->HasChildren())
	{
		RecursRenderDetail(triangle->m_T0, indices);
		RecursRenderDetail(triangle->m_T1, indices);
		RecursRenderDetail(triangle->m_T2, indices);
		RecursRenderDetail(triangle->m_T3, indices);
	}
	else
	{
		indices->Values.push_back(triangle->m_VertexIndices[0]);
