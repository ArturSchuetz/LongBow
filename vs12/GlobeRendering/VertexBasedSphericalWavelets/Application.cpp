...
bool Application::Init(void)
{
	...
	// Creating Window
	m_window = m_device->VCreateWindow(800, 600, "Vertex Based Wavelet Sphere", Renderer::WindowType::Windowed);
	if (m_window == nullptr)
	{
		return false;
	}
...
void Application::Update(float deltaTime)
{
	...
	if (m_keyboard->VIsPressed(Input::Key::K_UPARROW))
	{
		if (!m_keyUpPressed)
			m_Sphere.LevelDown();
	...
	else { m_keyUpPressed = false; }

	if (m_keyboard->VIsPressed(Input::Key::K_DOWNARROW))
	{
		if (!m_keyDownPressed)
			m_Sphere.LevelUp();
	...
	else { m_keyDownPressed = false; }

	if (m_keyboard->VIsPressed(Input::Key::K_Q))
	{
		if (!m_pressedQ)
			m_renderDetails = !m_renderDetails;
...
void Application::Render(void)
{
	...
	if (m_renderDetails)
	{
		m_Sphere.RenderDetail(m_contextOGL, m_camera);
	}
	else
	{
		m_Sphere.Render(m_contextOGL, m_camera);
	}
