...
void WaveletSphere::Init(bow::ShaderProgramPtr shaderProgram)
{
	m_shaderProgram = shaderProgram;
}

void WaveletSphere::Update(float deltaTime, bow::Camera* camera)
{
	...
	bow::Frustum<double> frustum = camera->CalculateFrustum(m_objectWorldMat);
	bow::Vector3<float> position = m_objectWorldMat.Inverse() * ((FirstPersonCamera*)(camera))->GetPosition();
...
void WaveletSphere::CaluclateScalarCoefficientsRecustive(WaveletFace* triangle, bow::Frustum<double>* frustum, bow::Camera* camera, bow::Vector3<float> position, int minLevel, int maxLevel, float threshold, std::ifstream* inputFile)
{
	if (maxLevel > 0)
	{
		...
		{		
			...
			if (m_realtime)
			{
				...
				bow::Vector3<float> center = ((triangle->m_p0->GetVertex(m_heightMultiplicator) + triangle->m_p1->GetVertex(m_heightMultiplicator) + triangle->m_p2->GetVertex(m_heightMultiplicator)) / 3);
				subdivide = fEdgeLength * 500 > (position - center).LengthSquared();
			}
			else
			{
				bow::Vector2<float> vec0 = camera->Transform3Dto2D(m_objectWorldMat * triangle->m_p0->GetVertex(m_heightMultiplicator));
				bow::Vector2<float> vec1 = camera->Transform3Dto2D(m_objectWorldMat * triangle->m_p1->GetVertex(m_heightMultiplicator));
				bow::Vector2<float> vec2 = camera->Transform3Dto2D(m_objectWorldMat * triangle->m_p2->GetVertex(m_heightMultiplicator));
...
void WaveletSphere::Render(bow::RenderContextPtr renderContext, bow::Texture2DPtr texture, bow::TextureSamplerPtr sampler, bow::Camera* camera, bool renderFilled)
{
	if (m_isDirty && m_updateVertexAttributes)
	{
		bow::MeshAttribute mesh;

		bow::VertexAttributeFloatVec4 *positionsAttribute = new bow::VertexAttributeFloatVec4("in_Position");
		mesh.AddAttribute(bow::VertexAttributePtr(positionsAttribute));

		bow::VertexAttributeFloatVec3 *normalsAttribute = new bow::VertexAttributeFloatVec3("in_Normal");
		mesh.AddAttribute(bow::VertexAttributePtr(normalsAttribute));
	...
	bow::Matrix3D<float> modelView = (bow::Matrix3D<float>)camera->CalculateWorldView(m_objectWorldMat);
	...
	m_shaderProgram->VSetUniform("u_ModelView", (bow::Matrix4x4<float>)modelView);
	m_shaderProgram->VSetUniform("u_LightDirection", (bow::Vector4<float>)(camera->CalculateView() * (bow::Vector4<float>(-50.0f, 25.0f, -100.0f, 1.0f))));
...
void WaveletSphere::CaluclateWaveletCoefficientsRecustive(WaveletFace* face, int level, const bow::Clipmap* heighMap, std::ofstream* outputFile)
{
	if (level > 0)
	{
	...
	else
	{
		...
		bow::Vector3<float> center = (face->m_p0->GetVertex(0.0f) + face->m_p1->GetVertex(0.0f) + face->m_p2->GetVertex(0.0f)) * 1.0f / 3.0f;
		center.Normalize();

		bow::Vector2<float> texCoord = bow::Vector2<float>((atan2(center.y, center.x) / (PI * 2.0f)) + 0.5f, (asin(center.z) / PI) + 0.5f);
...
void WaveletSphere::RecursRender(WaveletFace* face, bow::VertexAttributeFloatVec4 *positionsAttribute, bow::VertexAttributeFloatVec3 *normalsAttribute)
{
	...
	else
	{
		...
		{
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p0->GetVertex(heightMultiplicator), face->m_Value));
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p1->GetVertex(heightMultiplicator), face->m_Value));
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p2->GetVertex(heightMultiplicator), face->m_Value));
		...
		{
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p0->GetVertex(heightMultiplicator), face->m_p0->GetHeight()));
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p1->GetVertex(heightMultiplicator), face->m_p1->GetHeight()));
			positionsAttribute->Values.push_back(bow::Vector4<float>(face->m_p2->GetVertex(heightMultiplicator), face->m_p2->GetHeight()));
