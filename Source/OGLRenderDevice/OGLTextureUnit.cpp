...
namespace Bow {
	...
	void OGLTextureUnit::Validate()
	{
		...
		if (m_texture != nullptr)
        {
            if (m_textureSampler == nullptr)
            {
				LOG_ASSERT(false, "A texture sampler must be assigned to a texture unit with one or more bound textures.");
            }
			...
			if (m_texture->GetTarget() == GL_TEXTURE_RECTANGLE)
            {
                ...
                {
					LOG_ASSERT(false, "The texture sampler is incompatible with the rectangle texture bound to the same texture unit.  Rectangle textures only support linear and nearest minification filters.");
                }
                ...
                {
					LOG_ASSERT(false, "The texture sampler is incompatible with the rectangle texture bound to the same texture unit.  Rectangle textures do not support repeat or mirrored repeat wrap modes.");
                }
            }
        }
	}
