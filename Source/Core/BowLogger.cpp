...
EventLogger& EventLogger::GetInstance()
{
	...
	if(Instance.get() == nullptr)
	{
		...
		if(GetModuleFileName(GetCurrentModule(), buffer, bufSize)==bufSize)
		{
		...
		*(PathFindExtension(out)) = 0;
		...
		Instance->init(std::string(std::string((const char*)out) + std::string("Log.html")).c_str());
	}
	return *Instance.get();
}
...
{
	EventLogger::GetInstance().pushFunction(function);
}
...
bool EventLogger::init(const char* logName)
{
    ...
    if(!m_file)
    {
        ...
        char ext[_MAX_EXT];
        _splitpath_s( logName, drive, dir, fname, ext );
...
{ 
    ...
    int buf = _vsnprintf_s(buffer, MAX_DEBUG_LINE_LEN, format, args); 
#else
...
void EventLogger::logAssert(bool contidion, const char* file, long line, const char* description)
{
	if(!contidion)
	{
		logEvent(LOG_COLOR_RED | LOG_BOLD | LOG_UNDERLINE | LOG_SIZE_LARGE, std::string(std::string(description) + std::string(" - in file %s at line %u.")).c_str(), file, line);
		flush();
#ifdef _DEBUG
		assert(contidion);
#endif
...
{
    char buffer[MAX_DEBUG_LINE_LEN];
    int len = sprintf_s(buffer, "<font face=\"%s\" size=\"%d\" color=\"#%.2x%.2x%.2x\">", szFontName, iSize, red, green, blue);
...
void EventLogger::writeText(unsigned int flags, const char* lpszFormat, ...)
{
    ...
    int nBuf = _vsnprintf_s(buffer, MAX_DEBUG_LINE_LEN, lpszFormat, args); 
