...
FirstPersonCamera::FirstPersonCamera(const bow::Vector3<double> &cameraPosition, const bow::Vector3<double> &lookAtPoint, const bow::Vector3<double> &worldUp, unsigned int width, unsigned int height)
    : Camera(cameraPosition, lookAtPoint, worldUp, width, height), m_Position(cameraPosition), m_Up(worldUp)
{
    FN("FirstPersonCamera::FirstPersonCamera");
    OPTICK_EVENT();

    bow::Vector3<double> _lookAtPoint;
    if ((lookAtPoint - cameraPosition).Length() < 0.001f)
    {
        _lookAtPoint = cameraPosition + bow::Vector3<double>(0.0, 0.0, -1.0);
    }
    else
    {
    ...
    bow::Vector3<double> viewDirecton = (_lookAtPoint - cameraPosition).Normalized();
    m_Theta = std::acos(viewDirecton.y);
    m_Phi = std::atan2(viewDirecton.z, viewDirecton.x);
...
FirstPersonCamera::~FirstPersonCamera() { FN("FirstPersonCamera::~FirstPersonCamera"); }

void FirstPersonCamera::MoveForward(float deltaTime)
{
    FN("FirstPersonCamera::MoveForward");
    OPTICK_EVENT();
...
{
    FN("FirstPersonCamera::MoveBackward");
    OPTICK_EVENT();
...
{
    FN("FirstPersonCamera::MoveRight");
    OPTICK_EVENT();
...
{
    FN("FirstPersonCamera::calcViewDirection");
    OPTICK_EVENT();
