...
#include <LongBow/LongBow-version.h>
...
FileReader::FileReader() : m_pFile(nullptr) { m_pFile = (void *)new std::ifstream(); }
...
void FileReader::Close() { ((std::ifstream *)m_pFile)->close(); }
...
unsigned long FileReader::Tell() { return (unsigned long)((std::ifstream *)m_pFile)->tellg(); }

void FileReader::Seek(size_t offset) { ((std::ifstream *)m_pFile)->seekg(offset, std::ios::beg); }

void FileReader::Skip(long count) { ((std::ifstream *)m_pFile)->seekg(count, std::ios::cur); }
...
bool FileReader::EndOfFile() const { return ((std::ifstream *)m_pFile)->eof(); }
} // namespace bow
