...
FileWriter::FileWriter() : m_pFile(nullptr) { m_pFile = (void *)new std::ofstream(); }
...
void FileWriter::Close() { ((std::ofstream *)m_pFile)->close(); }
...
unsigned long FileWriter::Tell() { return (unsigned long)((std::ofstream *)m_pFile)->tellp(); }

void FileWriter::Seek(size_t offset) { ((std::ofstream *)m_pFile)->seekp(offset, std::ios::beg); }

void FileWriter::Skip(long count) { ((std::ofstream *)m_pFile)->seekp(count, std::ios::cur); }
...
bool FileWriter::EndOfFile() const { return ((std::ofstream *)m_pFile)->eof(); }
} // namespace bow
