...
public:
    ...
    Frustum(const Plane<T>& near, const Plane<T>& far, const Plane<T>& left,
const Plane<T>& right, const Plane<T>& top, const Plane<T>& bottom)
    {
    ...
    inline void Set(const Plane<T>& near, const Plane<T>& far, const Plane<T>&
left, const Plane<T>& right, const Plane<T>& top, const Plane<T>& bottom)
    {
        _near = near;
        _far = far;
        _left = left;
    ...
    inline Frustum* CalculateFrustum(const Matrix4x4<T>& modelViewProjection)
    {
        ...
        {
            T norm = sqrt(_planes[i].Normal.x * _planes[i].Normal.x +
_planes[i].Normal.y * _planes[i].Normal.y + _planes[i].Normal.z *
_planes[i].Normal.z); if (norm != 0.0)
            {
    ...
    inline bool IsVisible(const Vector3<float>& position)
    {
        for (size_t i = 0; i < 6; i++)
        {
            float f = (position.DotP(_planes[i].Normal)) + _planes[i].Distance;
    ...
    inline bool IsTriangleVisible(const Vector3<float>& p0, const
Vector3<float>& p1, const Vector3<float>& p2)
    {
        ...
        for (size_t i = 0; i < 6; i++)
        {
            float f = (p0.DotP(_planes[i].Normal)) + _planes[i].Distance;
            if (f < 0.0)
            {
        ...
        for (size_t i = 0; i < 6; i++)
        {
            float f = (p1.DotP(_planes[i].Normal)) + _planes[i].Distance;
            if (f < 0.0)
            {
        ...
        for (size_t i = 0; i < 6; i++)
        {
            float f = (p2.DotP(_planes[i].Normal)) + _planes[i].Distance;
            if (f < 0.0)
            {
        ...
        if (((p0_culling & CullingPosition::Near) != 0	&& (p1_culling &
CullingPosition::Near) != 0	&& (p2_culling & CullingPosition::Near) != 0) ||
            ((p0_culling & CullingPosition::Far) != 0	&& (p1_culling &
CullingPosition::Far) != 0		&& (p2_culling & CullingPosition::Far) != 0) ||
            ((p0_culling & CullingPosition::Right) != 0 && (p1_culling &
CullingPosition::Right) != 0	&& (p2_culling & CullingPosition::Right) != 0)
||
            ((p0_culling & CullingPosition::Left) != 0	&& (p1_culling &
CullingPosition::Left) != 0	&& (p2_culling & CullingPosition::Left) != 0) ||
            ((p0_culling & CullingPosition::Top) != 0	&& (p1_culling &
CullingPosition::Top) != 0		&& (p2_culling & CullingPosition::Top) != 0) ||
            ((p0_culling & CullingPosition::Bottom) != 0 && (p1_culling &
CullingPosition::Bottom) != 0 && (p2_culling & CullingPosition::Bottom) != 0))
        {
            return false;
        }
        else
