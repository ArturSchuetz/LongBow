...
class MaterialCollection : public Resource
{
  ...
  public:
    ...
    void NameMaterial(const std::string &name, uint16_t index);
    void UnnameMaterial(const std::string &name);

    uint16_t GetMaterialIndex(const std::string &name) const;

    std::vector<Material *> GetMaterials() const
    {
        FN("MaterialCollection::GetMaterials");
    ...
    void DestroyMaterial(uint16_t index);

  private:
    // you shall not copy
    MaterialCollection(const MaterialCollection &other) = delete;
    MaterialCollection &operator=(const MaterialCollection &other) = delete;
    ...
    */
    ...
    std::map<std::string, uint16_t> m_materialNameMap;
};
} // namespace bow
