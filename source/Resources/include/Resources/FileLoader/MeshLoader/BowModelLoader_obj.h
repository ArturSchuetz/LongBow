...
class ModelLoader_obj
{
  ...
  private:
    // you shall not copy
    ModelLoader_obj(const ModelLoader_obj &other) = delete;
    ModelLoader_obj &operator=(const ModelLoader_obj &other) = delete;

    bool exportFaceGroupToShape(Mesh *mesh, const std::vector<vertex_index> &face);
    uint32_t getVertexIndex(Mesh *mesh, index_t index);
