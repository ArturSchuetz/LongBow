...
class PointCloudLoader_xtc
{
  ...
  private:
    // you shall not copy
    PointCloudLoader_xtc(const PointCloudLoader_xtc &other) = delete;
    PointCloudLoader_xtc &operator=(const PointCloudLoader_xtc &other) = delete;

    std::istream &safeGetline(std::istream &is, std::string &t);

    inline void parseReal6(float *x, float *y, float *z, float *r, float *g, float *b, const char **token, const float default_x = 0.0, const float default_y = 0.0, const float default_z = 0.0, const float default_r = 1.0,
                           const float default_g = 1.0, const float default_b = 1.0);
