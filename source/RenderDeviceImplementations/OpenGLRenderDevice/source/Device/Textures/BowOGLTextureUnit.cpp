#include <OpenGLRenderDevice/Device/Textures/BowOGLTextureUnit.h>
...
OGLTextureUnit::OGLTextureUnit(uint32_t index, ICleanableObserver *observer) : m_textureUnitIndex(index), m_textureUnit(GL_TEXTURE0 + index), m_observer(observer), m_texture(nullptr), m_textureSampler(nullptr), m_DirtyFlags(DirtyFlags::None)
{
    FN("OGLTextureUnit::OGLTextureUnit");
...
{
    FN("OGLTextureUnit::CleanLastTextureUnit");
...
{
    FN("OGLTextureUnit::Validate");

    if (m_texture != nullptr)
    {
        if (m_textureSampler == nullptr)
        {
            LOG_FATAL("A texture sampler must be assigned to a texture unit "
                      "with one or more bound textures.");
        }
        ...
        if (m_texture->GetTarget() == GL_TEXTURE_RECTANGLE)
        {
            ...
            {
                LOG_FATAL("The texture sampler is incompatible with the "
                          "rectangle texture bound to the same texture "
                          "unit.  Rectangle textures only support linear and "
                          "nearest minification filters.");
            }
            ...
            if (m_textureSampler->WrapS == TextureWrap::Repeat || m_textureSampler->WrapS == TextureWrap::MirroredRepeat || m_textureSampler->WrapT == TextureWrap::Repeat || m_textureSampler->WrapT == TextureWrap::MirroredRepeat)
            {
                LOG_FATAL("The texture sampler is incompatible with the "
                          "rectangle texture bound to the same texture "
                          "unit.  Rectangle textures do not support repeat or "
