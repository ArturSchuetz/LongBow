...
{
  public:
    virtual void VCopyFromSystemMemory(void *bufferInSystemMemory, int lengthInBytes) { VCopyFromSystemMemory(bufferInSystemMemory, 0, lengthInBytes); }

    virtual void VCopyFromSystemMemory(void *bufferInSystemMemory, int destinationOffsetInBytes, int lengthInBytes) = 0;

    virtual std::shared_ptr<void> VCopyToSystemMemory() { return VCopyToSystemMemory(0, VGetSizeInBytes()); }
