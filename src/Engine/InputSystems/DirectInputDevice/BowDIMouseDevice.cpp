...
namespace Bow {
	namespace Input {
		...
		bool DIMouseDevice::Init(void)
		{
			memset(m_Pressed, false, sizeof(m_Pressed));
			m_x = m_y = 0;

			if (!DIInputDeviceBase::Init(GUID_SysMouse, &c_dfDIMouse))
			{
				return false;
			}
		...
		Core::Vector3<long> DIMouseDevice::VGetRelativePosition() const
		{
			return Core::Vector3<long>(m_relativeX, m_relativeY, m_relativeScrollWheel);
		}
